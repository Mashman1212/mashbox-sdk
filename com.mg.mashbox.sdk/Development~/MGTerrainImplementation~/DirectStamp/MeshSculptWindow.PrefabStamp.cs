...
namespace MashBoxSDK.MapTools
{
    public sealed partial class MeshSculptWindow
    {
        ...
        void DrawStampSource()
        {
            ...
            m_StampNormals = EditorGUILayout.Toggle(new GUIContent("Stamp Normals", "Transfer material and geometry normals in world space and blend into the far-range normal map."), m_StampNormals);
            if (m_StampColour || m_StampNormals)
            {
                EditorGUILayout.HelpBox("Maps apply on release into existing baked terrain maps, preserving their resolution. RGB transfers unlit base colour; normals follow the stamp surface. Undo restores the previous maps.", MessageType.None);
            }
        }
        ...
        void CommitStampAppearance()
        {
            ...
            try { PrefabStampAppearance.Apply(m_PrefabStamp, m_MeshStampBrush, m_AppearanceDabs, m_StampColour, m_StampNormals); }
