...
namespace MashBoxSDK.Maps.TerrainSystem
{
    public sealed partial class MGTerrain
    {
        ...
        internal int WorldScaledDetailDemand(float nearScale, float distantScale, int budget)
        {
            if (!m_WorldPendingDraw) return 0;
            long count = 0;
            foreach (var visible in m_VisibleDensityDetails)
                count += Mathf.FloorToInt(GetVisibleDensityDetailInstanceCount(visible)
                    * (visible.densityLod == 0 ? nearScale : distantScale));
            return (int)Math.Min(count, budget);
        }
