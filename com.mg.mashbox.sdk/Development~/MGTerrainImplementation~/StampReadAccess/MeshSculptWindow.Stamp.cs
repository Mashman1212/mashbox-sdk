...
namespace MashBoxSDK.MapTools
{
    public sealed partial class MeshSculptWindow
    {
        ...
        void DrawMeshStampSettings(bool compact = false)
        {
            ...
            if (mesh != m_StampMesh) { m_StampMesh = mesh; m_MeshStampBrush = null; m_FailedStampMesh = null; m_StampReadError = null; }
            m_StampHeight = Mathf.Max(0.001f, EditorGUILayout.FloatField(compact ? "Height (m)" : "Stamp Height (m)", m_StampHeight));
            m_StampRotation = EditorGUILayout.Slider(compact ? "Rotation" : "Stamp Rotation", m_StampRotation, 0f, 360f);
            ...
            if (!string.IsNullOrEmpty(m_StampReadError))
            {
                ...
                if (GUILayout.Button("Retry Reading Stamp")) { m_FailedStampMesh = null; m_StampReadError = null; SceneView.RepaintAll(); }
            }
        }
        ...
        {
            if (m_StampMesh == null || m_StampMesh == m_FailedStampMesh || m_StampMesh.bounds.size.y < 0.00001f)
                return false;
            if (m_MeshStampBrush == null || m_MeshStampBrush.Source != m_StampMesh)
            {
                try { m_MeshStampBrush = new MeshStampBrush(m_StampMesh); m_StampReadError = null; }
                catch (System.Exception error)
                {
                    m_FailedStampMesh = m_StampMesh;
