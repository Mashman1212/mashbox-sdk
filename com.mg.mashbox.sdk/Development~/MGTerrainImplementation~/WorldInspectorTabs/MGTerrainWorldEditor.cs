    ...
    public sealed partial class MGTerrainWorldEditor : Editor
    {
        ...
        void DrawTabBar()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
                ...
                {
                    Rect rect = GUILayoutUtility.GetRect(70, 34, GUILayout.ExpandWidth(true));
                    bool selected = (int)m_Tab == i;
                    if (GUI.Toggle(rect, selected, new GUIContent("    " + labels[i], tips[i]), EditorStyles.miniButton) && !selected)
                    {
                        m_AddTiles = false;
                        m_ShowChunk = false;
                    ...
                    if (Event.current.type == EventType.Repaint)
                        DrawTabIcon(new Rect(rect.x + 7, rect.center.y - 8, 16, 16), i, selected);
                }
        }
