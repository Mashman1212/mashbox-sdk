        ...
        void DrawStampSource()
        {
            ...
            if (m_StampColour || m_StampNormals)
            {
                ...
                if (setupTiles.Count > 0)
                {
                    ...
                    if (GUILayout.Button(new GUIContent("Generate Missing Appearance Maps", "Bake RGB and world-space normal maps from the terrain. Existing assigned maps are preserved. Uses the world's bake resolution and exposure.")))
                    {
                        int completed = 0;
                        try
                        {
                            foreach (var tile in setupTiles) { MGTerrainStampMapSetup.Generate(tile); completed++; }
