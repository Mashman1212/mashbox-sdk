...
namespace MashBoxSDK.MapTools
{
    public sealed partial class MGTerrainEditor
    {
        ...
        void DrawTileAuthoring(MGTerrain terrain)
        {
            ...
            using (new EditorGUI.DisabledScope(Application.isPlaying))
            {
                ...
                m_StampSource = (MeshCollider)EditorGUILayout.ObjectField("Mesh Stamp Surface", m_StampSource, typeof(MeshCollider), true);
                ...
                EditorGUILayout.HelpBox("Position the source over the destination. Conform samples Unity Terrain at world X/Z; mesh stamps project down onto the assigned non-convex MeshCollider. Only the overlap changes. Connected tile edges stay joined. Sources are left intact; Ctrl+Z undoes the operation.", MessageType.Info);
                ...
                using (new EditorGUI.DisabledScope(m_StampSource == null || m_StampSource.sharedMesh == null))
                    if (GUILayout.Button("Stamp Mesh Height")) RunTileAction(() => MGTerrainTileAuthoring.Conform(terrain, null, m_StampSource, m_StampBlend, m_StampOffset));
            }
        }
    ...
    internal static class MGTerrainTileAuthoring
    {
        ...
        internal static MGTerrain AddTile(MGTerrain source, Vector2Int direction)
        {
            ...
            var xs = vertices.Select(v => v.x).Distinct().OrderBy(x => x).ToArray();
            var zs = vertices.Select(v => v.z).Distinct().OrderBy(z => z).ToArray();
            if ((long)xs.Length * zs.Length != vertices.Length || xs.Length < 2 || zs.Length < 2)
                throw new InvalidOperationException("Add Tile requires a rectangular height grid. Conform and stamping can also edit other terrain meshes.");
            var heights = new Dictionary<Vector2, float>();
            foreach (var v in vertices) heights.Add(new Vector2(v.x, v.z), v.y);
            var output = new Vector3[vertices.Length];
            var uv = new Vector2[vertices.Length];
            for (int z = 0; z < zs.Length; z++)
                for (int x = 0; x < xs.Length; x++)
                {
                    float y = heights[new Vector2(direction.x == 0 ? xs[x] : direction.x > 0 ? xs[xs.Length - 1] : xs[0],
                        direction.y == 0 ? zs[z] : direction.y > 0 ? zs[zs.Length - 1] : zs[0])];
                    int i = z * xs.Length + x;
                    output[i] = new Vector3(xs[x], y, zs[z]);
                    uv[i] = new Vector2((xs[x] - xs[0]) / (xs[xs.Length - 1] - xs[0]), (zs[z] - zs[0]) / (zs[zs.Length - 1] - zs[0]));
                }
            ...
            try
            {
                ...
                var result = new Mesh { name = go.name, indexFormat = mesh.indexFormat, vertices = output, uv = uv, triangles = triangles };
