    ...
    public sealed partial class MeshSculptWindow : EditorWindow
    {
        ...
        bool HandleBrushAdjustment(Event current, int controlId, SceneView sceneView)
        {
            ...
            else if (m_IsAdjustingBrush && current.type == EventType.MouseDrag && current.button == 2)
            {
                ...
                else if (IsMeshStamp) m_StampHeight = Mathf.Clamp(m_StampHeight * Mathf.Exp(-current.delta.y * .01f), .001f, 100000f);
            ...
            EditorGUI.ProgressBar(
                ...
                IsMeshStamp ? $"Height  {m_StampHeight:0.00} m   (drag vertically)" : $"Strength  {GetAdjustmentStrength():0.00}   (drag vertically)");
            Handles.EndGUI();
            return true;
        }
