...
namespace MashBoxSDK.MapTools
{
    public sealed partial class MGTerrainEditor
    {
        ...
        void DrawFlattenTile(MGTerrain terrain)
        {
            ...
            using (new EditorGUI.DisabledScope(Application.isPlaying || terrain.MeshFilter == null
                || terrain.MeshFilter.sharedMesh == null || !terrain.MeshFilter.sharedMesh.isReadable))
            {
                m_FlattenHeight = EditorGUILayout.FloatField(new GUIContent("Flatten Height (m)",
                    "World-space elevation for the entire selected tile. Defaults to the tile origin's height."), m_FlattenHeight.Value);
                using (new EditorGUI.DisabledScope(float.IsNaN(m_FlattenHeight.Value) || float.IsInfinity(m_FlattenHeight.Value)))
                    if (GUILayout.Button(new GUIContent("Flatten Tile", "Flatten every vertex of this tile to the specified height. Neighbouring tiles are unchanged. Undo restores the previous shape.")))
                        RunTileAction(() => MGTerrainTileAuthoring.Flatten(terrain, m_FlattenHeight.Value));
            }
        }
