...
namespace MashBoxSDK.MapTools
{
    public sealed partial class MGTerrainEditor
    {
        ...
        void DrawDistantMorph(MGTerrain terrain)
        {
            ...
            m_ApplyDistantMorph = EditorGUILayout.Toggle(new GUIContent("Apply Bake to Trail Shader", "Bake height and appearance maps for the existing terrain mesh, without generating a separate distant mesh or material."), m_ApplyDistantMorph);
        ...
        void ApplyDistantMorph(MGTerrain terrain, Texture2D height)
        {
            ...
            var previousSlots = renderer.sharedMaterials;
            transaction.OnRollback(() => renderer.sharedMaterials = previousSlots);
            var source = MGTerrainAssetStore.Material(terrain, transaction);
            ...
            if (height == null || (height.format != TextureFormat.RFloat && height.format != TextureFormat.R16) || !height.isReadable)
                throw new InvalidOperationException("Choose the readable R16 (or legacy RFloat) _Height.asset produced by the distant surface bake.");
            ...
            var colour = AssetDatabase.LoadAssetAtPath<Texture2D>(stem + ".asset") ?? AssetDatabase.LoadAssetAtPath<Texture2D>(stem + ".png");
            var normal = AssetDatabase.LoadAssetAtPath<Texture2D>(stem + "_NormalWS.asset") ?? AssetDatabase.LoadAssetAtPath<Texture2D>(stem + "_NormalWS.png");
            ...
            var material = source;
            Undo.RecordObject(material, "Apply Distant Terrain Morph");
