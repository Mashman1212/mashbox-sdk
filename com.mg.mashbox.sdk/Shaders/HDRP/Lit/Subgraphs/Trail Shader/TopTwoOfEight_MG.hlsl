...
void LightmapUV_half(float2 uv, out float2 lightmapUV)
{
    lightmapUV = uv * unity_LightmapST.xy + unity_LightmapST.zw;
}
...
float shEvaluateDiffuseL1Geomerics(float L0, float3 L1, float3 n)
{
    ...
    return R0 * (a + (1.0f - a) * (p + 1.0f) * pow(max(q, 1e-5), p));
}
...
{
    float3 nL1x = tex2D(_RNM0, lightmapUV).rgb * 2 - 1;
    float3 nL1y = tex2D(_RNM1, lightmapUV).rgb * 2 - 1;
    float3 nL1z = tex2D(_RNM2, lightmapUV).rgb * 2 - 1;
    float3 L1x = nL1x * L0 * 2;
    float3 L1y = nL1y * L0 * 2;
    float3 L1z = nL1z * L0 * 2;

    float lumaL0 = dot(L0, float3(0.2126, 0.7152, 0.0722));
    float lumaL1x = dot(L1x, float3(0.2126, 0.7152, 0.0722));
    float lumaL1y = dot(L1y, float3(0.2126, 0.7152, 0.0722));
    float lumaL1z = dot(L1z, float3(0.2126, 0.7152, 0.0722));
    ...
    float regularLumaSH = dot(sh, float3(0.2126, 0.7152, 0.0722));
                                                                                                    ...
                                                                                                    out float3 diffuseSH, out float3 specularSH)
{
    ...
    float regularLumaSH = dot(diffuseSH, float3(0.2126, 0.7152, 0.0722));
                                                                                                    ...
                                                                                                    out float3 diffuseSH, out float3 specularSH)
{
    ...
    // Output Non Linear Light Probe Sampling if no lightmap present
    float3 Loh = float3(unity_SHAr.w, unity_SHAg.w, unity_SHAb.w);
    diffuseSH.r = shEvaluateDiffuseL1Geomerics(Loh.r, unity_SHAr.xyz, normalWorld);
    diffuseSH.g = shEvaluateDiffuseL1Geomerics(Loh.g, unity_SHAg.xyz, normalWorld);
    diffuseSH.b = shEvaluateDiffuseL1Geomerics(Loh.b, unity_SHAb.xyz, normalWorld);
    diffuseSH.rgb = max(diffuseSH.rgb, 0);
...
void BakerySpecMonoSH_half(
    ...
    half metalness,
    out half3 diffuseSH,
    out half3 specularSH)
{
#ifdef LIGHTMAP_ON
#ifdef DIRLIGHTMAP_COMBINED
    half3 L0 = unity_Lightmap.Sample(samplerunity_Lightmap, lightmapUV);
    half3 dominantDir = unity_LightmapInd.Sample(samplerunity_Lightmap, lightmapUV);
    ...
    half lumaL0 = dot(L0, half3(0.2126, 0.7152, 0.0722));
    half lumaL1x = dot(L1x, half3(0.2126, 0.7152, 0.0722));
    half lumaL1y = dot(L1y, half3(0.2126, 0.7152, 0.0722));
    half lumaL1z = dot(L1z, half3(0.2126, 0.7152, 0.0722));

    half lumaSH = shEvaluateDiffuseL1Geomerics(
        lumaL0,
        half3(lumaL1x, lumaL1y, lumaL1z),
        normalWorld);
    ...
    half regularLumaSH = dot(diffuseSH, half3(0.2126, 0.7152, 0.0722));

    diffuseSH *= lerp(
        (half)1,
        lumaSH / max(regularLumaSH, (half)1e-5),
        saturate(regularLumaSH * 16));

    diffuseSH = max(diffuseSH, (half)0.0);
    ...
    specularSH = max(spec * specularSH, (half)0.0);
    ...
    half reflectivity = max(
        max(specularColor.r, specularColor.g),
        specularColor.b);

    half grazingTerm = saturate(smoothness + reflectivity);

    half nv = 1.0h - saturate(dot(normalWorld, viewDir));
    half nv2 = nv * nv;
    half fresnel = nv * nv2 * nv2;

    half3 fresnel3 = lerp(
        specularColor,
        half3(grazingTerm, grazingTerm, grazingTerm),
        fresnel);
    ...
    // Fallback probe
    half3 Loh = half3(unity_SHAr.w, unity_SHAg.w, unity_SHAb.w);

    diffuseSH.r = shEvaluateDiffuseL1Geomerics(Loh.r, unity_SHAr.xyz, normalWorld);
    diffuseSH.g = shEvaluateDiffuseL1Geomerics(Loh.g, unity_SHAg.xyz, normalWorld);
    diffuseSH.b = shEvaluateDiffuseL1Geomerics(Loh.b, unity_SHAb.xyz, normalWorld);

    diffuseSH.rgb = max(diffuseSH.rgb, (half)0);
...
{
    bool isGlobal = dot(abs(_VolumeInvSize), float3(1,1,1)) == 0;
